...
export function KanbanBoard({
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={onDragEnd}>
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-4 items-start">
...
function Column({
  ...
  const columnTheme: Record<string, typeof defaultTheme> = {
    ...
    "In Progress": {
      ...
      glow: "shadow-[0_0_28px_rgba(255,159,28,0.12)]",
    ...
    Completed: {
      ...
      glow: "shadow-[0_0_28px_rgba(0,255,153,0.12)]",
      ...
      <div className="relative z-10 mb-2 flex items-center justify-between">
        <div>
          ...
          <div className="mt-0.5 flex items-center gap-1.5">
            <span className={`breathe h-1.5 w-1.5 rounded-full ${theme.dot}`} />
            <h2 className="text-base font-black text-white">{status}</h2>
          </div>
        </div>

        <span
          className={`
            breathe rounded-full border px-2 py-0.5 text-[11px] font-bold
            ${theme.badge}
          `}
        >
          {tasks.length}
  ...
  return (
    <div
      ...
      className={`
        ...
        ${
          isDragging
            ? "z-50 scale-[1.02] opacity-85 shadow-[0_0_35px_rgba(0,229,255,0.22)]"
